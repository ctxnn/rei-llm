...
class GPT(nn.Module):
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int = 200, temperature: float = 1.0,
        ...
        """
        start with prompt "idx" -> crop to block size -> take tokens last logit -> forward pass -> temperature smoothening -> topk or topp filtering -> softmax -> sample 1 token -> append to idx -> repeat
        """
        from utils import top_k_top_p_filtering
        self.eval()
        #  if the prompt is empty, start with a newline byte (10)
        if idx.size(1) == 0:
